...
cancer_data <- read.csv("Adv_cancer_cleaned.csv")
dim(cancer_data)
...
actual <- as.factor(cancer_new$Advanced.Cancer[1:200])
input <- input[1:200]
one_shot_c7 <- data.frame(actual,pred,response2,response1,input)

#Confusion matrix
library(caret)
confusionMatrix(data=one_shot_c7$pred,reference = one_shot_c7$actual,mode = "prec_recall",positive = "1")

# Create a list of messages with the one-shot prompt including label/response
one_shot_message <- list(
  list(role = "system", content = "Please act as a healthcare professional and provide a response based on the summary using the following instructions: Identify if the patient has cancer or not. If there is clear evidence of cancer, respond 'Yes.' If there is no strong evidence of cancer or if the evidence is unclear, respond 'No.' Please provide a concise response as either 'Yes' or 'No'."),
  list(role = "user", content = input_summary),
